...
### Out of the 7 empty function below
    ...
    def dict_mapping(self, key_value, main_value):
        ...
        for sub_key, sub_value in key_value.items():
            try:
                name, val = self.get_function_based_on_type(sub_value, main_value[sub_key])
...
class GetPages():
    def __init__(self):
        ...
        self.browser_header = {
            ...
            ':scheme': 'https',
            ':path:': '/issuers/company_issuers_a_en.htm',
            ':method:': 'GET',
            ':authority': 'www.sedar.com'
    ...
    def getpages_pdf(self, searchquery, search_column, xpath, url, headers=None, data=None, method='GET', file_base_url=None, multiple_tables=True, pages='all', stream=True, area=None):
        df = None
        try:
            fd = self.get_file(xpath, url, headers=headers, data=data, method=method)
